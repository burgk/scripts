...
if [[ ${distro} = "Redhat" ]] || [[ ${distro} = "CentOS" ]] || [[ ${distro} = "Oracle" ]]; then
  ...
  if [[ ${majversion} = "12" ]] || [[ ${majversion} = "15" ]]; then
    supportedver="true"
    return
  else
    validate_distroversion_err
  ...
  if [[ ${majversion} = "8" ]] || [[ ${majversion} = "9" ]] || [[ ${majversion} = "10" ]]; then
    supportedver="true"
    return
  else
    validate_distroversion_err
  ...
  if [[ ${majversion} = "14" ]] || [[ ${majversion} = "16" ]] || [[ ${majversion} = "18" ]] || [[ ${majversion} = "20" ]]; then
    supportedver="true"
    return
  fi
elif [[ ${distro} = "Amazon" ]]; then
  if [[ ${majversion} = "2" ]] || [[ ${majversion} = "2018.03" ]]; then
    supportedver="true"
    return
  else
    validate_distroversion_err
...
case ${distro} in
  ...
  ;;
  Debian | debian)
    if [[ ${majversion} = 8 && ${host64} = "true" ]] || [[ ${majversion} = 8 && ${host64} = "false" ]]; then
      supportedarch="true"
    elif [[ ${majversion} = 9 && ${host64} = "true" ]] || [[ ${majversion} = 9 && ${host64} = "false" ]]; then
      supportedarch="true"
  ...
  Ubuntu)
    if [[ ${majversion} = 14 && ${host64} = "true" ]]; then
      ...
      supportedarch="true"
    elif [[ ${majversion} = 20 && ${host64} = "true" ]]; then
      supportedarch="true"
    else
      supportedarch="false"
    fi
...
if [[ ${supported_distro} = "true" ]] && [[ ${supportedver} = "true" ]] && [[ ${supportedarch} = "true" ]]; then
  ...
  ;;
  Debian | debian)
    if [[ ${majversion} = 10 && ${host64} = "true" ]]; then
      installpkg=${debian10_64}
      installmethod="systemd"
      clientname="taniumclient"
    elif [[ ${majversion} = 9 && ${host64} = "true" ]]; then
  ...
  ;;
  Ubuntu)
    if [[ ${majversion} = 20 ]]; then
      installpkg=${ubuntu20_64}
      installmethod="systemd"
      clientname="taniumclient"
    elif [[ ${majversion} = 18 ]]; then
