...
distro=$(grep "^Red Hat" /etc/system-release | awk '{print $1$2}')
if [[ ${distro} = "RedHat" ]]; then
  echo "this is RedHat linux, checking version..."
else
  echo "Not RedHat, exiting..."
  exit 0
fi

version=$(grep ^VERSION_ID /etc/os-release | awk -F'=' '{print $2}' | awk '{print substr($0,2,1)}')
if [[ ${version} != "8" ]]; then
  echo "Not RedHat 8, exiting..."
  exit 0
fi

PROFILE=${1:-Level 1}

if [ -f /home/harden1/* ]; then 
...
if [ "$PROFILE" = "Level 1" ] || [ "$PROFILE" = "Level 2" ]; then
  ...
  echo Ensure\ noexec\ option\ set\ on\ /tmp\ partition not configured
   if ! grep -q '/tmp' /etc/fstab ; then echo 'tmpfs /tmp tmpfs nosuid,nodev,noexec 0 0' >> /etc/fstab ; else egrep -q "^(\s*\S+\s+)/tmp (\s+\S+\s+\S+)(\s+\S+\s+\S+)(\s*#.*)?\s*$" /etc/fstab && sed -ri "s/^(\s*\S+\s+)\/dev\/shm(\s+\S+\s+\S+)(\s+\S+\s+\S+)(\s*#.*)?\s*$/\1\/dev\/shm\2,noexec,nosuid,nodev\3\4/" /etc/fstab ;fi
  ...
  echo Ensure\ nodev\ option\ set\ on\ /dev/shm\ partition not configured
   if ! grep -q '/dev/shm' /etc/fstab ; then echo 'tmpfs /dev/shm tmpfs nosuid,nodev,noexec 0 0' >> /etc/fstab ; else egrep -q "^(\s*\S+\s+)/dev/shm(\s+\S+\s+\S+)(\s+\S+\s+\S+)(\s*#.*)?\s*$" /etc/fstab && sed -ri "s/^(\s*\S+\s+)\/dev\/shm(\s+\S+\s+\S+)(\s+\S+\s+\S+)(\s*#.*)?\s*$/\1\/dev\/shm\2,noexec,nosuid,nodev\3\4/" /etc/fstab ;fi
  ...
  touch /etc/modprobe.d/usb-storeage.conf
  echo "install usb-storage /bin/true" >> /etc/modprobe.d/usb-storeage.conf
 ...
 # done
  ...
  sysctl -w net.ipv4.ip_config.all.send_redirects=0
  sysctl -w net.ipv4.ip_config.default.send_redirects=0
  sysctl -w net.ipv4.route.flush=1
  ...
  egrep -q "^(\s*)net.ipv4.conf.all.accept_redirects\s*=\s*\S+(\s*#.*)?\s*$" /etc/sysctl.conf && sed -ri "s/^(\s*)net.ipv4.conf.all.accept_redirects\s*=\s*\S+(\s*#.*)?\s*$/\1net.ipv4.conf.all.accept_redirects = 0\2/" /etc/sysctl.conf || echo "net.ipv4.conf.all.accept_redirects = 0" >> /etc/sysctl.conf
  sysctl -w net.ipv4.ip_config.all.accept_redirects=0
  sysctl -w net.ipv4.ip_config.default.accept_redirects=0
  sysctl -w net.ipv4.route.flush=1
  ...
  sysctl -w net.ipv4.ip_config.all.secure_redirects=0
  sysctl -w net.ipv4.ip_config.default.secure_redirects=0
  sysctl -w net.ipv4.route.flush=1
  ...
  sysctl -w net.ipv4.ip_config.all.log.martians=1
  sysctl -w net.ipv4.ip_config.default.log_martians=1
  sysctl -w net.ipv4.route.flush=1
  ...
  echo
  echo \*\*\*\* Ensure\ nftables\ service\ is\ enabled
  ...
  # Ensure journald is configured to send logs to ryslog
  echo
  ...
  echo
  echo \*\*\*\* Ensure\ SSH\ Maxstartup\ is\ configured
  ...
  if ! (grep -q 'AllowGroups' /etc/ssh/sshd_config); then echo 'AllowGroups adm wheel admins' >> /etc/ssh/sshd_config ;fi
  ...
  # Create custom authselect profile and configured
  echo
  echo echo \*\*\*\* Create\ custom\ authselect\ profile\ and\ configured
...
mkdir /home/harden1
touch /home/harden1/harden1.txt.`date +"%d-%m-%Y"`
