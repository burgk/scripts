...
while [[ "${valid_stype}" = "false" ]]; do
  ...
  case "${user_tmptype}" in
    ...
    p | P)
      ...
      echo -e "Yes, back slashes (\), not the expected"
      echo -e "forward slashes (/)"
      echo -e "Partial paths are ok but should include"
      echo -e "at least the \ifs\<accesszone> part."
      echo -e "Remember, a more exact search will reduce"
      echo -e "the number of irrelevant matches."
      echo -e "Wildcards are not necessary."
      echo -e "Also, remember the share we are searching"
      echo -e "will need to be translated to an Isilon path,"
      echo -e "e.g. at CDOT, the share name \\public\ORGDEV is at path"
      echo -e "/ifs/CDOT/fileshares/Public/17_O/ORGDEV"
